        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            if (_currentSelectedMenudIndex!=-1) {
                ...
                [self animateLeftTableView:_leftTableView rightTableView:_rightTableView show:NO complete:^{
                    ...
                    [self animateIdicator:self->_indicators[tapIndex] background:self->_backGroundView collectionView:collectionView title:self->_titles[tapIndex] forward:YES complecte:^{
                        self->_show = YES;
                    }];
                }];
            } else{
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            if (_currentSelectedMenudIndex!=-1) {
                ...
                [self animateCollectionView:_collectionView show:NO complete:^{
                    ...
                    [self animateIdicator:self->_indicators[tapIndex] background:self->_backGroundView leftTableView:self->_leftTableView rightTableView:self->_rightTableView title:self->_titles[tapIndex] forward:YES complecte:^{
                        self->_show = YES;
                    }];
                }];
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)(void))complete {
    ...
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (leftTableView) {
                leftTableView.frame = CGRectMake(self->_origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width*ratio, tableViewHeight);
            }
            if (rightTableView) {
                rightTableView.frame = CGRectMake(self->_origin.x+leftTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, self.frame.size.width*(1-ratio), tableViewHeight);
            }
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            if (leftTableView) {
                leftTableView.frame = CGRectMake(self->_origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width*ratio, 0);
            }
            if (rightTableView) {
                rightTableView.frame = CGRectMake(self->_origin.x+leftTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, self.frame.size.width*(1-ratio), 0);
            }
...
- (void)animateCollectionView:(UICollectionView *)collectionView show:(BOOL)show complete:(void(^)(void))complete {
    ...
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (collectionView) {
                collectionView.frame = CGRectMake(self->_origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, collectionViewHeight);
            }
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            if (collectionView) {
                collectionView.frame = CGRectMake(self->_origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 0);
            }
        } completion:^(BOOL finished) {
